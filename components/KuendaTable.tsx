...
export function KuendaTableHeader({ children }: { children: ReactNode }) {
	return (
		<thead>
			{children}
		</thead>
...
export function KuendaTableHeaderRow({ children }: { children: ReactNode }) {
	return (
...
export function KuendaTableHeaderRowCell({
	text,
	align
}: {
	text: string | number,
	...
	return (
		<th className={`py-3 px-6 ${align ? align : 'text-left'}`}>{text}</th>
	)
}

export function KuendaTableRow({ children }: { children: ReactNode }) {
	return (
...
export function KuendaTableCell({
	children,
	text,
	align,
	offLabel,
	...
	children?: ReactNode, text?: string | number, align?: 'text-left' | 'text-center' | 'text-right', offLabel?: string, onLabel?: string
}) {
	return (
		<td className={`py-3 px-6 whitespace-nowrap ${align ? align : 'text-left'}`}>
			<div className={`flex items-center font-medium text-gray-900 ${align ? align : 'text-left'}`}>
				{!offLabel && onLabel && <div title={onLabel} className="w-2 h-2 rounded-full mr-2 bg-red-600"></div>}
				{!onLabel && offLabel && <div title={offLabel} className="w-2 h-2 rounded-full mr-2 bg-green-600"></div>}
				{children ? children : <></>}
				{text && <span className={`font-medium ${align ? align : 'text-left'}`}>{String(text)}</span>}
			</div>
		</td>
	)
}

export default function KuendaTable({ header, children }: { header: ReactNode, children: ReactNode }) {
	return (
		<>
